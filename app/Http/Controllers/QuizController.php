...
class QuizController extends Controller
{
  ...
  public function getSelect(Request $request)
  {
    ...
    $data = Quiz::select('id','quiz_type_id','title')->orWhere('title','like',"%$param%")->get()->sortBy('quiz_type_id');
    $list = [];
      foreach ($data as $key => $value) {
          $list[] = [
              'id'=>$value->id,
              'quiz_type_id'=>$value->quiz_type_id,
              'text'=>$value->title
          ];
      }
  ...
  {
    $data = Quiz::select('id','quiz_type_id','title')->where('quiz_type_id',$id)->get();
    $list = [];
      foreach ($data as $key => $value) {
          $list[] = [
              'id'=>$value->id,
